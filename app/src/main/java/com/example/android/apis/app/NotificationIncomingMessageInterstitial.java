 ...
 * that is posted by {@link NotificationIncomingMessage}.  It allows the user to switch
 * to the app in its appropriate state if they want.
 */
public class NotificationIncomingMessageInterstitial extends Activity {
    @Override
...
//BEGIN_INCLUDE(app_launch)
    ...
    void switchToApp() {
        ...
        CharSequence from = getIntent().getCharSequenceExtra(NotificationIncomingMessageView.KEY_FROM);
        CharSequence msg = getIntent().getCharSequenceExtra(NotificationIncomingMessageView.KEY_MESSAGE);
        // Build the new activity stack, launch it, and finish this UI.
        Intent[] stack = NotificationIncomingMessage.makeMessageIntentStack(this, from, msg);
