...
public class TimerObj implements Parcelable {
    ...
    private int getNextTimerId(Context context) {
        ...
        synchronized (TimerObj.class) {
            /**
             * M: Modify the timerid's init value to avoid the problem that the
             * timer id is the same as alarm id, that cancel the timeup
             * notitication also canel the alarm head up notification meanwhile. @{
             */
    ...
    public static void resetTimersInSharedPrefs(SharedPreferences prefs) {
        ...
        for (TimerObj timer : timers) {
            ...
            ///M: we should clear the sharedPreference not rewrite it @{
